...
THREE.MD2CharacterComplex = function () {
	...
	this.loadParts = function ( config ) {
		...
		this.loadCounter = config.weaponskin.length * 2 + config.skins.length + 1;

		// SKINS

		this.skinsBody = loadTextures( config.baseUrl + "skins/", config.skins );
		this.skinsWeapon = loadTextures( config.baseUrl + "skins/", config.weaponskin );
		...
		//for ( var i = 0; i < config.weapons.length; i ++ ) {
			...
			loader.load( config.baseUrl + config.weapon, generateCallback( 0, config.weapon ) );
	...
	this.updateMovementModel = function ( delta ) {
		...
		pos.x = this.root.position.x
		pos.z = this.root.position.z
	};
